...
export default function UploadImage(props) {
	...
	const [showModalCameraOrGallery, setShowModalCameraOrGallery] = useState(
		false
	)
	...
	const uploadFromCamera = async () => {
		const resultCameraPermissions = await Permissions.askAsync(
			Permissions.CAMERA
		)

		if (resultCameraPermissions.status == 'denied') {
			toastRef.current.show(
				'Negaste los permisos para la camara. Para aceptarlos, tenés que ir a ajustes del telefono'
			)
		} else {
	...
	const uploadFromGallery = async () => {
		const resultGalleryPermissions = await Permissions.askAsync(
			Permissions.CAMERA_ROLL
		)
		if (resultGalleryPermissions.status == 'denied') {
			toastRef.current.show(
				'Negaste los permisos a la galería. Para aceptarlos, tenés que ir a ajustes del telefono'
			)
		} else {
	...
	return (
		<View style={styles.viewImage}>
			...
			/>
			<Modal
				isVisible={showModalCameraOrGallery}
				setIsVisible={setShowModalCameraOrGallery}
			>
				<CameraOrGallery
