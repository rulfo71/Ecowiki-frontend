		...
		;(async () => {
			try {
				...
				if (toastRef.current) {
					// console.log(`toastRef: ${JSON.stringify(toastRef)}`);
					toastRef.current.show(
						'Upss! Hubo un error buscando los productos. Intentá de nuevo mas tarde',
						3000,
						() => {
							navigation.navigate(Constants.Navigations.home)
						}
	...
	const getProducts = async () => {
		try {
			...
			if (response.length > 0) {
				...
				var startProductNameResponse = response[
					response.length - 1
				].displayName.toLowerCase()
	...
	return (
		<View style={styles.container}>
			{isEmpty(products) || products.length == 0 ? (
				...
				<Login redirectTo={Constants.Navigations.ProductStack.voteProducts} />
			) : (
				<Swiper
					ref={(swiper) => {
						this.swiper = swiper
					...
					onSwipedRight={(cardIndex) => {
						addVote(products[cardIndex])
					}}
					onSwipedLeft={(cardIndex) => {
						subtractVote(products[cardIndex])
					}}
